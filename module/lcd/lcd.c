...
static void lcd_clear_display(struct i2c_client *client) ;


/*
 * Get temperature for DHT11 Module
 *
 *

static ssize_t read_temperature_from_dht11(char *temperature_str, size_t size)
{
    ssize_t len;
    char temperature_path[] = "/sys/dht11/temperature";
    int fd = open(temperature_path, O_RDONLY);
    ...
    len = read(fd, temperature_str, size);
...
/**
 * Function to clear the screen before writing 
 *
 */
static void lcd_clear_display(struct i2c_client *client) 
{
    lcd_send_cmd(client, LCD_CMD_CLEARDISPLAY);
    ...
    msleep(2); // Wait for the clear operation to complete
}
...
static void lcd_init(struct i2c_client *client)
{
    ...
    sprintf((char*)msg + msg_len, " t: %d C", temperature);
